...
'''
Definición de la formulación dfj. Parámetros:

1.model: Modelo a modificar.

2. x: Variables del modelo.

3. n: número de nodos.

Este método solo modifica al modelo para permitir las 
"LazyConstrains" o restricciones en tiempo de ejecución.

La eliminación de subtours se realiza en métodos posteriores. 
...
Este método genera una formulación básica de ATSP (problema del viajante
asimétrico). Los parámetros de entrada son:

1. function_subtours: Formulación usada para "romper" los subtours.
En nuestro caso solo usaremos la dfj.

2.num_nodes: Número de nodos que intervienen.

3.distance_matrix: matriz de distancias correspondiente.
...
'''
Función para identificar subtours. Parámetros:

1. edges: conjunto de "nodos".

2. n: tamaño del conjunto.
...
'''
Método que elimina subtours añadiendo restricciones en tiempo de ejecución
"LazyConstrains"
'''
...
'''
Método que trae los datos de entrada del
archivo donde se encuentren. Parámetros:

1.path: ruta donde se encuentre el archivo.
'''
...
Este método se usará una vez se halla optimizado el modelo.
Interpreta las variables del modelo y deduce el orden en el que
se han de recorrer los contenedores. Parámetros:

1.model: Modelo ya optimizado.

2.contenedores: Lista con los conenedores de la ruta, en orden coherente con la matriz
de distancias.
...
'''
Método para llevar a un json el contenido de una variable. Parámetros:

1.path: ruta donde se ha de guardar el archivo.

2.output_var: contenido del archivo.
'''
...
'''
Este método convierte una matriz en una variable tipo diccionario
que entiende el método atsp_basic. Parámetros:

1.matrix: matriz a convertir.
'''
...
'''
Método principal donde se realiza la optimización.
'''
def main():
    ruta='*****'
    ...
    model.setParam('TIME_LIMIT', 36000)
